...
// НЕ генерируем AKIA-подобные значения: строка AKIA + 16 символов неотличима от
// настоящего AWS Access Key ID, и GitHub Push Protection блокирует такой пуш.
// ASCII-детекция SECRET_AWS покрыта юнит-тестом со сборкой ключа в рантайме.
...
{
  const jwt = makeJwt();
  const kv = chars(B64, 24);
  add(`В логах CI найден токен ${jwt}, а также запись token=${kv} — ротируйте немедленно.`, [
